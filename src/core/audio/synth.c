...
static const uint16_t NES_APU_NOISE_PERIODS[16] = {4,	8,	 16,  32,  64,	96,	  128,	160,
...
void synth_generate_samples(struct Synth *synth, float *buffer, uint32_t num_samples)
{
	...
	for (int v = 0; v < MAX_VOICES; v++)
	{
		...
		for (uint32_t i = 0; i < num_samples; i++)
		{
			...
			float samples_elapsed = voice->elapsed_ms * (float)synth->sample_rate / 1000.0f;

			float final_amplitude = voice->amplitude;
			if (voice->decay != 0)
			{
				float decay_per_sample = (voice->amplitude * (float)abs(voice->decay)) / 4800000.0f;
